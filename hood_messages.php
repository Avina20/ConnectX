...
$sql = "SELECT 
        ...
        WHERE 
            ug.user_id = :user_id AND t.group_id = 4
        GROUP BY 
            t.thread_id, 
            t.title, 
            t.thread_user_id, 
...
<?php if (!empty($result)) : ?>
    <?php foreach ($result as $row) : ?>
        <div class="message-container">
            <h5 style="cursor: pointer;" onclick="window.location.href='hood_thread_details.php?thread_id=<?php echo $row['thread_id']; ?>'"><?php echo htmlspecialchars($row['title']); ?></h5>
            <p class="meta">
